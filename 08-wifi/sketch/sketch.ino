...
void setup() {
  ...
  if ( u8g.getMode() == U8G_MODE_R3G3B2 ) {
    PRINTLN("U8G_MODE_R3G3B2");
    u8g.setColorIndex(255);     // white
  }
  else if ( u8g.getMode() == U8G_MODE_GRAY2BIT ) {
    PRINTLN("U8G_MODE_GRAY2BIT");
...
void draw(char const *s) {
  ...
  do {
    ...
    u8g.drawStr(64 - u8g.getStrWidth(s) / 2, 32 + 15, s);
  } while( u8g.nextPage() );
}
...
void doSun(int n) {
  ...
  String s;
  if (n == 1) {
    s = (day ? "'" : ".") + format(t1 + diff) + " ";
  } else {
    s = " " + format(t2 - diff) + (day ? "." : "'");
  }
  PRINTLN(s);
  draw(s.c_str());
}
